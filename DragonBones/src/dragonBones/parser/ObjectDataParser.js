...
export class ObjectDataParser extends DataParser {
	...
	_parseArmature(rawData, scale) {
		...
		if (DataParser.AABB in rawData) {
			...
			armature.aabb.width =
				ObjectDataParser._getNumber(rawAABB, DataParser.WIDTH, 0.0) * armature.scale;
			armature.aabb.height =
				ObjectDataParser._getNumber(rawAABB, DataParser.HEIGHT, 0.0) * armature.scale;
		}
	...
	_parseTimeline(
		...
		if (rawData !== null) {
			this._timelineArray[timelineOffset + BinaryOffset.TimelineScale] = Math.round(
				ObjectDataParser._getNumber(rawData, DataParser.SCALE, 1.0) * 100
			);
			this._timelineArray[timelineOffset + BinaryOffset.TimelineOffset] = Math.round(
	...
	_parseBoneRotateFrame(rawData, frameStart, frameCount) {
		...
		this._frameFloatArray[frameFloatOffset++] =
			ObjectDataParser._getNumber(rawData, DataParser.SKEW, 0.0) * Transform.DEG_RAD;
	...
	_parseTransform(rawData, transform, scale) {
		...
		if (DataParser.ROTATE in rawData || DataParser.SKEW in rawData) {
			transform.rotation = Transform.normalizeRadian(
				ObjectDataParser._getNumber(rawData, DataParser.ROTATE, 0.0) * Transform.DEG_RAD
			);
			...
			transform.rotation = Transform.normalizeRadian(
				ObjectDataParser._getNumber(rawData, DataParser.SKEW_Y, 0.0) * Transform.DEG_RAD
			);
			transform.skew =
				Transform.normalizeRadian(
					ObjectDataParser._getNumber(rawData, DataParser.SKEW_X, 0.0) * Transform.DEG_RAD
				) - transform.rotation;
		}
	...
	parseTextureAtlasData(rawData, textureAtlasData, scale = 1.0) {
		...
		textureAtlasData.scale =
			scale === 1.0 ? 1.0 / ObjectDataParser._getNumber(rawData, DataParser.SCALE, 1.0) : scale;
